...
JOIN aluno ON treinos.id_aluno = aluno.id_aluno
WHERE (treinos.classe_treino = 'c' AND treinos.id_aluno = $id_aluno) OR (treinos.classe_treino = 'F' AND treinos.id_aluno = $id_aluno)
...
if ($result->num_rows > 0) {
 echo "<table style='border-collapse: collapse; width: 15%; border: 3px solid black;'>";
    ...
    while($row = $result->fetch_assoc()) {
       echo "<tr>";
        echo "<td style='border: 3px solid black; width: 20%;'>".$row["EXERCICIO"]."</td>";
